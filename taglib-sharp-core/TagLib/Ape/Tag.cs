...
namespace TagLib.Ape
{
    ...
    public class Tag : TagLib.Tag, IEnumerable<string>
    {
        ...
        /// </summary>
        private static readonly string[] _pictureItemNames = new string[] {
            "Cover Art (other)",
            "Cover Art (icon)",
        ...
        public Tag(ByteVector data)
        {
            ...
            Parse(data.Mid((int)(data.Count - _footer.TagSize),
                (int)(_footer.TagSize - Footer.Size)));
        }
        ...
        public void RemoveItem(string key)
        {
            ...
            for (int i = _items.Count - 1; i >= 0; i--)
                if (key.Equals(_items[i].Key, comparison))
                    _items.RemoveAt(i);
        }
        ...
        ///    cref="_items" /> at which the item appears, or -1 if the
        ///    item was not found.
        /// </returns>
        /// <remarks>
        ...
        public override IPicture[] Pictures
        {
            get
            {
                ...
                for (int i = 0; i < _pictureItemNames.Length;
                    i++)
                {
                    Item item = GetItem(
                        _pictureItemNames[i]);
            ...
            set
            {
                foreach (string itemName in _pictureItemNames)
                    RemoveItem(itemName);
                ...
                foreach (IPicture pic in value)
                {
                    ...
                    if (type >= _pictureItemNames.Length)
                        type = 0;

                    string name = _pictureItemNames[type];
