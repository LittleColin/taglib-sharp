...
namespace TagLib.Ape
{
	...
	public struct Footer : IEquatable<Footer>
    {
		...
		public Footer(ByteVector data)
        {
            ...
            _tagSize = data.Mid(12, 4).ToUInt(false);
            _itemCount = data.Mid(16, 4).ToUInt(false);
            _flags = (FooterFlags)data.Mid(20, 4).ToUInt(false);
        }
		...
		/// </value>
		public uint Version => _version == 0 ? 2000 : _version;
		...
		private ByteVector Render(bool isHeader)
        {
            ...
            // add the tag size
            v.Add(ByteVector.FromUInt(_tagSize, false));

            // add the item count
            v.Add(ByteVector.FromUInt(_itemCount, false));
		...
		public override int GetHashCode()
        {
            unchecked
            {
                return (int)((uint)_flags ^ _tagSize ^
                    _itemCount ^ _version);
            }
        }
