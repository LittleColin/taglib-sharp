...
namespace TagLib.Id3v2
{
    ...
    public class Tag : TagLib.Tag, IEnumerable<Frame>
    {
        ...
        /// </summary>
        private static bool _forceDefaultStringType = false;

        /// <summary>
        ///    Indicates whether or not all tags should be saved in
        ///    <see cref="_defaultVersion" />.
        /// </summary>
        private static bool _forceDefaultVersion = false;
        ...
        /// </summary>
        private readonly List<Frame> _frameList = new List<Frame>();

        /// <summary>
        ///    Contains the tag's extended header.
        /// </summary>
        private ExtendedHeader _extendedHeader = null;
        ...
        public override void CopyTo(TagLib.Tag target, bool overwrite)
        {
            ...
            List<Frame> frames = new List<Frame>(_frameList);
        ...
        public void RemoveFrames(ByteVector ident)
        {
            ...
            for (int i = _frameList.Count - 1; i >= 0; i--)
                if (_frameList[i].FrameId.Equals(ident))
        ...
        protected void Parse(ByteVector data)
        {
            ...
            tdrc.Text = new string[] { tdrcText.ToString() };
        }
        ...
        private void MakeFirstOfType(Frame frame)
        {
            ...
            for (int i = 0; i < _frameList.Count; i++)
            {
                if (swapping == null)
                {
                    if (_frameList[i].FrameId.Equals(type))
                        swapping = frame;
                    else
                        continue;
                }

                Frame tmp = _frameList[i];
                _frameList[i] = swapping;
                swapping = tmp;
