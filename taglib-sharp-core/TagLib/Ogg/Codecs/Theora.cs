...
namespace TagLib.Ogg.Codecs
{
    ...
    public class Theora : Codec, IVideoCodec
    {
        ...
        /// </summary>
        private static readonly ByteVector _id = "theora";
                                              ...
                                              long lastGranularPosition)
        {
            return TimeSpan.FromSeconds(
                _header.GranuleTime(lastGranularPosition) -
                _header.GranuleTime(firstGranularPosition));
        }
        ...
        /// </value>
        public int VideoWidth => _header.Width;
        ...
        ///    video represented by the current instance.
        /// </value>
        public int VideoHeight => _header.Height;
        ...
        {
            if (packet.Count <= _id.Count || packet[0] < 0x80)
                return -1;
        ...
        private struct HeaderPacket
        {
            ...
            public readonly int KeyframeGranuleShift;

            public HeaderPacket(ByteVector data)
            {
                MajorVersion = data[7];
                MinorVersion = data[8];
                RevisionVersion = data[9];
                ...
                Height = (int)data.Mid(17, 3).ToUInt(); // Frame Height.
                                                        // Offset X.
                                                        // Offset Y.
                FpsNumerator = (int)data.Mid(22, 4).ToUInt();
                FpsDenominator = (int)data.Mid(26, 4).ToUInt();
                // Aspect Numerator.
                // Aspect Denominator.
                // Colorspace.
                // Target bitrate.
                ushort lastBits = data.Mid(40, 2).ToUShort();
                KeyframeGranuleShift = (lastBits >> 5) & 0x1F;
            }
