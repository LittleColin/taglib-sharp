...
namespace TagLib.Asf
{
    ...
    public class ContentDescriptionObject : Object
    {
        ...
        /// </exception>
        public ContentDescriptionObject(File file, long position) : base(file, position)
        {
            ...
            ushort titleLength = file.ReadWord();
            ushort authorLength = file.ReadWord();
            ushort copyrightLength = file.ReadWord();
            ushort descriptionLength = file.ReadWord();
            ushort ratingLength = file.ReadWord();

            _title = file.ReadUnicode(titleLength);
            _author = file.ReadUnicode(authorLength);
            _copyright = file.ReadUnicode(copyrightLength);
            _description = file.ReadUnicode(descriptionLength);
            _rating = file.ReadUnicode(ratingLength);
        }
        ...
        public string Title
        {
            get { return _title.Length == 0 ? null : _title; }
            set
            {
        ...
        {
            get { return _author.Length == 0 ? null : _author; }
            set
            {
                _author = string.IsNullOrEmpty(value) ?
                    string.Empty : value;
            }
        }
        ...
        {
            get { return _copyright.Length == 0 ? null : _copyright; }
            set
            {
                _copyright = string.IsNullOrEmpty(value) ?
                    string.Empty : value;
            }
        }
        ...
        {
            get { return _rating.Length == 0 ? null : _rating; }
            set
            {
                _rating = string.IsNullOrEmpty(value) ?
                    string.Empty : value;
            }
        }
        ...
        /// </value>
        public bool IsEmpty => _title.Length == 0 &&
                               _author.Length == 0 &&
                               _copyright.Length == 0 &&
        ...
        {
            ByteVector titleBytes = RenderUnicode(_title);
            ByteVector authorBytes = RenderUnicode(_author);
            ByteVector copyrightBytes = RenderUnicode(_copyright);
            ByteVector descriptionBytes =
                RenderUnicode(_description);
            ByteVector ratingBytes = RenderUnicode(_rating);

            ByteVector output = RenderWord((ushort)
                titleBytes.Count);
            output.Add(RenderWord((ushort)authorBytes.Count));
            output.Add(RenderWord((ushort)copyrightBytes.Count));
            output.Add(RenderWord((ushort)
                descriptionBytes.Count));
            output.Add(RenderWord((ushort)ratingBytes.Count));
            output.Add(titleBytes);
            output.Add(authorBytes);
            output.Add(copyrightBytes);
            output.Add(descriptionBytes);
            output.Add(ratingBytes);
