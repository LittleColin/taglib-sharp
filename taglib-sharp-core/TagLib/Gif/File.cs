...
namespace TagLib.Gif
{
    ...
    public class File : ImageBlockFile
    {
        ...
        /// </summary>
        protected static readonly string Version87A = "87a";

        /// <summary>
        ///    String which is used to indicate version the gif file format version 89a
        /// </summary>
        protected static readonly string Version89A = "89a";
        ...
        /// </remarks>
        private static readonly byte[] XmpMagicTrailer = new byte[] {
        ...
        private void ReadHeader()
        {
            ...
            var readVersion = data.Mid(3, 3).ToString();
            if (readVersion == Version87A || readVersion == Version89A)
                _version = readVersion;
            else
                throw new UnsupportedFormatException(
                    string.Format("Only GIF versions 87a and 89a are currently supported, but not: {0}", readVersion));

            // Read Image Size (little endian)
            _width = data.Mid(6, 2).ToUShort(false);
            _height = data.Mid(8, 2).ToUShort(false);
        ...
        private ByteVector RenderGifCommentBlock()
        {
            ...
            GifCommentTag commentTag = GetTag(TagTypes.GifComment) as GifCommentTag;
            if (commentTag == null)
                return null;

            string comment = commentTag.Comment;
            if (comment == null)
                return null;

            ByteVector commentData = new ByteVector();

            // Add Extension Introducer (0x21) and Comment Label (0xFE)
            commentData.Add(new byte[] { 0x21, 0xFE });

            // Add data of comment in sub-blocks of max length 256.
            ByteVector commentBytes = new ByteVector(comment);
            byte blockMax = 255;
            for (int start = 0; start < commentBytes.Count; start += blockMax)
            {
                byte blockLength = (byte)Math.Min(commentBytes.Count - start, blockMax);

                commentData.Add(blockLength);
                commentData.Add(commentBytes.Mid(start, blockLength));
            }
            commentData.Add(new byte[] { 0x00 });
        ...
        {
            ByteVector commentBlock = RenderGifCommentBlock();
            ...
            if (commentBlock != null && _version != Version89A)
            {
                Insert(Version89A, 3, Version89A.Length);
            }

            // now, only metadata is stored at the beginning of the file, and we can overwrite it.
            ByteVector metadataBlocks = new ByteVector();
            metadataBlocks.Add(commentBlock);

            SaveMetadata(metadataBlocks, _startOfBlocks);
        }
        ...
        /// </remarks>
        private void SkipColorTable(byte packedData)
        {
            ...
            // table.
            if ((packedData & 0x80) == 0x80)
            {
                // 2^(size + 1) for each color.
                int tableSize = 3 * (1 << ((packedData & 0x07) + 1));

                // and simply skip the color table
                ByteVector colorTable = ReadBlock(tableSize);

                if (colorTable.Count != tableSize)
