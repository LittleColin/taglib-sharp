...
namespace TagLib.IFD.Entries
{
    ...
    public class StripOffsetsIfdEntry : ArrayIfdEntry<uint>
    {
        ...
        /// </value>
        private readonly uint[] _byteCounts;
        ...
        /// </param>
        public StripOffsetsIfdEntry(ushort tag, uint[] values, uint[] byteCounts, File file) : base(tag)
        {
            Values = values;
            _byteCounts = byteCounts;
            _file = file;

            if (values.Length != byteCounts.Length)
        ...
        /// </returns>
        public override ByteVector Render(bool isBigendian, uint offset, out ushort type, out uint count)
        {
            ...
            uint dataOffset = offset + (uint)(4 * Values.Length);

            for (int i = 0; i < Values.Length; i++)
            {
                uint newOffset = (uint)(dataOffset + data.Count);

                _file.Seek(Values[i], SeekOrigin.Begin);
                data.Add(_file.ReadBlock((int)_byteCounts[i]));

                // update strip offset data to new offset
                Values[i] = newOffset;

                offsetData.Add(ByteVector.FromUInt(newOffset, isBigendian));
            }
