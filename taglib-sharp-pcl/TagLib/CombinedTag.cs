...
namespace TagLib
{
    ...
    public class CombinedTag : Tag
    {
        ...
        public override TagTypes TagTypes
        {
            get
            {
                return _tags.Where(tag => tag != null).Aggregate(TagTypes.None, (current, tag) => current | tag.TagTypes);
            }
        }
        ...
        public override string Title
        {
            get
            {
                return _tags.Select(tag => tag?.Title).FirstOrDefault(value => value != null);
            }
            set
            {
                foreach (Tag tag in _tags.Where(tag => tag != null))
                    tag.Title = value;
            }
        }
        ...
        public override string TitleSort
        {
            get
            {
                return _tags.Select(tag => tag?.TitleSort).FirstOrDefault(value => !string.IsNullOrEmpty(value));
            }
            set
            {
                foreach (Tag tag in _tags.Where(tag => tag != null))
        ...
        public override uint Track
        {
            get
            {
                return (from tag in _tags where tag != null select tag.Track).FirstOrDefault(value => value != 0);
            }
            set
            {
                foreach (Tag tag in _tags.Where(tag => tag != null))
        ...
        public override uint TrackCount
        {
            get
            {
                return (from tag in _tags where tag != null select tag.TrackCount).FirstOrDefault(value => value != 0);
            }
            set
            {
                foreach (Tag tag in _tags.Where(tag => tag != null))
        ...
        public override uint Year
        {
            get
            {
                return (from tag in _tags where tag != null select tag.Year).FirstOrDefault(value => value != 0);
            }
            set
            {
                foreach (Tag tag in _tags.Where(tag => tag != null))
