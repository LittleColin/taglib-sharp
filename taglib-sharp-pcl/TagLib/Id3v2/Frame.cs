...
namespace TagLib.Id3v2
{
    ...
    public abstract class Frame
    {
        ...
        public virtual ByteVector Render(byte version)
        {
            ...
            ByteVector fieldData = RenderFields(version);
            ...
            if ((Flags & FrameFlags.Unsynchronisation) != 0)
                SynchData.UnsynchByteVector(fieldData);

            if (frontData.Count > 0)
                fieldData.Insert(0, frontData);

            _header.FrameSize = (uint)fieldData.Count;
            ByteVector headerData = _header.Render(version);
            headerData.Add(fieldData);
                                        ...
                                        int offset, byte version)
        {
            ...
            int dataOffset = offset + (int)FrameHeader.Size(version);
            ...
            if ((Flags & FrameFlags.GroupingIdentity) != 0)
            {
                if (frameData.Count >= dataOffset)
                    throw new CorruptFileException(
                        "Frame data incomplete.");
                _groupId = frameData[dataOffset++];
                dataLength--;
            }
            ...
            if ((Flags & FrameFlags.Encryption) != 0)
            {
                if (frameData.Count >= dataOffset)
                    throw new CorruptFileException(
                        "Frame data incomplete.");
                _encryptionId = frameData[dataOffset++];
                dataLength--;
            }

            dataLength = Math.Min(dataLength, frameData.Count - dataOffset);
            if (dataLength < 0)
                throw new CorruptFileException(
                    "Frame size less than zero.");

            ByteVector data = frameData.Mid(dataOffset,
                dataLength);

            if ((Flags & FrameFlags.Unsynchronisation) != 0)
            {
                int beforeLength = data.Count;
                SynchData.ResynchByteVector(data);
                dataLength -= (data.Count - beforeLength);
            }
