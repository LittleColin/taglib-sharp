...
namespace TagLib.Id3v2
{
    ...
    public class RelativeVolumeFrame : Frame
    {
        ...
        {
            return _channels[(int)type].VolumeAdjustmentIndex;
        }
        ...
        public void SetVolumeAdjustmentIndex(ChannelType type,
                                              short index)
        {
            _channels[(int)type].VolumeAdjustmentIndex = index;
        }
        ...
        {
            return _channels[(int)type].VolumeAdjustment;
        }
        ...
        public void SetVolumeAdjustment(ChannelType type,
                                         float adjustment)
        {
            _channels[(int)type].VolumeAdjustment = adjustment;
        }
        ...
        {
            return _channels[(int)type].PeakVolumeIndex;
        }
        ...
        {
            _channels[(int)type].PeakVolumeIndex = index;
        }
        ...
        {
            return _channels[(int)type].PeakVolume;
        }
        ...
        {
            _channels[(int)type].PeakVolume = peak;
        }
                                             ...
                                             byte version)
        {
            ...
            _identification = data.ToString(StringType.Latin1, 0,
                pos++);
            ...
            while (pos <= data.Count - 4)
            {
                ...
                unchecked
                {
                    _channels[type].VolumeAdjustmentIndex =
                        (short)data.Mid(pos,
                            2).ToUShort();
                }
                pos += 2;
                ...
                _channels[type].PeakVolumeIndex = data.Mid(pos,
                    bytes).ToULong();
                pos += bytes;
            }
        }
        ...
        protected override ByteVector RenderFields(byte version)
        {
            ...
            data.Add(ByteVector.FromString(_identification,
                StringType.Latin1));
            ...
            for (byte i = 0; i < 9; i++)
            {
                ...
                for (byte j = 0; j < 64; j++)
                    if ((_channels[i].PeakVolumeIndex &
                        (1UL << j)) != 0)
                        bits = (byte)(j + 1);
        ...
        public override Frame Clone()
        {
            RelativeVolumeFrame frame =
                new RelativeVolumeFrame(_identification);
            for (int i = 0; i < 9; i++)
