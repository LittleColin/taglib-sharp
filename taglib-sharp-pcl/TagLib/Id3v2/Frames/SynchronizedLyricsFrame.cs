...
namespace TagLib.Id3v2
{
    ...
    public class SynchronisedLyricsFrame : Frame
    {
        ...
        /// </summary>
        private SynchedTextType _lyricsType = SynchedTextType.Other;

        /// <summary>
        ///    Contains the text.
        /// </summary>
        private SynchedText[] _text = new SynchedText[0];
        ...
        public string Language
        {
            get
            {
                return (_language != null && _language.Length > 2)
                    ? _language.Substring(0, 3) : "XXX";
            }
            set { _language = value; }
        }
        ...
        {
            get { return _lyricsType; }
            set { _lyricsType = value; }
        }
        ...
        public SynchedText[] Text
        {
            ...
            set
            {
                _text = value == null ? new SynchedText[0] :
                    value;
            }
        }
                                                            ...
                                                            SynchedTextType type)
        {
            ...
            int bestValue = -1;
            SynchronisedLyricsFrame bestFrame = null;
                                             ...
                                             byte version)
        {
            ...
            _timestampFormat = (TimestampFormat)data[4];
            _lyricsType = (SynchedTextType)data[5];

            ByteVector delim = ByteVector.TextDelimiter(
                _encoding);
            int delimIndex = data.Find(delim, 6, delim.Count);

            if (delimIndex < 0)
                throw new CorruptFileException(
                    "Text delimiter expected.");

            _description = data.ToString(_encoding, 6,
                delimIndex - 6);

            int offset = delimIndex + delim.Count;
            List<SynchedText> l = new List<SynchedText>();

            while (offset + delim.Count + 4 < data.Count)
            {
                delimIndex = data.Find(delim, offset,
                    delim.Count);
                ...
                string text = data.ToString(_encoding, offset,
                    delimIndex - offset);
                offset = delimIndex + delim.Count;
        ...
        protected override ByteVector RenderFields(byte version)
        {
            ...
            v.Add(ByteVector.FromString(Language,
                StringType.Latin1));
            v.Add((byte)_timestampFormat);
            v.Add((byte)_lyricsType);
        ...
        {
            SynchronisedLyricsFrame frame =
                new SynchronisedLyricsFrame(_description,
                    _language, _lyricsType, _encoding);
            frame._timestampFormat = _timestampFormat;
            frame._text = (SynchedText[])_text.Clone();
            return frame;
        }
    }
