...
namespace TagLib.Id3v2
{
    ...
    public class TextInformationFrame : Frame
    {
        ...
        /// </summary>
        private string[] _textFields = new string[0];
        ...
        /// <summary>
        ///    Contains the ID3v2 version of <see cref="_rawData" />.
        /// </summary>
        ...
        /// </param>
        public TextInformationFrame(ByteVector ident)
            : this(ident, Tag.DefaultEncoding)
        {
        }
        ...
        protected void ParseRawData()
        {
            ...
            {
                fieldList.AddRange(data.ToStrings(_encoding, 1));
            }
            else if (data.Count > 1 && !data.Mid(1,
              delim.Count).Equals(delim))
            {
                string value = data.ToString(_encoding, 1,
                    data.Count - 1);

                // Truncate values containing NULL bytes
                int nullIndex = value.IndexOf('\x00');
                if (nullIndex >= 0)
                {
                ...
                {
                    fieldList.AddRange(value.Split('/'));
                }
            ...
            while (fieldList.Count != 0 &&
                string.IsNullOrEmpty(fieldList[
                    fieldList.Count - 1]))
                fieldList.RemoveAt(fieldList.Count - 1);

            _textFields = fieldList.ToArray();
        }
        ...
        public override Frame Clone()
        {
            TextInformationFrame frame =
                ...
                new UserTextInformationFrame(null, _encoding) :
                new TextInformationFrame(FrameId, _encoding);
    ...
    public class UserTextInformationFrame : TextInformationFrame
    {
                                                    ...
                                                    bool caseSensitive)
        {
            ...
            UserTextInformationFrame newFrame =
