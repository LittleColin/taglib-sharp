...
namespace TagLib.Mpeg
{
    ...
    public class File : NonContainer.File
    {
        ...
        {
            Tag t = ((NonContainer.Tag)Tag).GetTag(type);
        ...
        {
            ByteVector packet = new ByteVector(MarkerStart) { (byte)marker };
        ...
        protected override Properties ReadProperties(long start,
                                                      ...
                                                      ReadStyle propertiesStyle)
        {
            TimeSpan duration = _startTime == null ?
                TimeSpan.Zero : TimeSpan.FromSeconds(
                    _endTime - (double)_startTime);

            return new Properties(duration, _videoHeader,
                _audioHeader);
        }
        ...
        protected void ReadSystemFile(long position)
        {
            ...
            for (int i = 0; i < sanityLimit && (_startTime == null ||
                !_audioFound || !_videoFound); i++)
            {
        ...
        private void ReadAudioPacket(ref long position)
        {
            ...
            if (!_audioFound)
                _audioFound = AudioHeader.Find(
                    out _audioHeader, this, position + 15,
                    length - 9);
            position += length;
        }
        ...
        private void ReadSystemSyncPacket(ref long position)
        {
            int packetSize;
            Seek(position + 4);
            byte versionInfo = ReadBlock(1)[0];
        ...
        private void ReadVideoPacket(ref long position)
        {
            ...
            while (!_videoFound && offset < position + length)
                ...
                {
                    _videoHeader = new VideoHeader(this,
                        offset + 4);
                    _videoFound = true;
                }
                else
