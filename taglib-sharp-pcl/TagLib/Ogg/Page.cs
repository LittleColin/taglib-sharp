...
namespace TagLib.Ogg
{
    ...
    public class Page
    {
        ...
        {
            _header = header;
            _packets = new ByteVectorCollection();
        }
        ...
        {
            file.Seek(position + _header.Size);

            foreach (int packetSize in _header.PacketSizes)
                _packets.Add(file.ReadBlock(packetSize));
        }
            ...
            : this(header)
        {
            ...
            _packets = new ByteVectorCollection(packets);
        ...
        {
            ByteVector data = _header.Render();

            foreach (ByteVector v in _packets)
                data.Add(v);
        ...
        /// </value>
        public ByteVector[] Packets => _packets.ToArray();
        ...
        /// </value>
        public uint Size => _header.Size + _header.DataSize;
                                                     ...
                                                     IDictionary<uint, int> shiftTable)
        {
            ...
            while (position < file.Length - 27)
            {
                ...
                {
                    file.Seek(position);
                    ByteVector pageData = file.ReadBlock(size);

                    ByteVector newData = ByteVector.FromUInt(
                        (uint)(header.PageSequenceNumber +
                        shiftTable[header.StreamSerialNumber]),
                        false);

                    for (int i = 18; i < 22; i++)
                        pageData[i] = newData[i - 18];
                    for (int i = 22; i < 26; i++)
                        pageData[i] = 0;

                    newData.Add(ByteVector.FromUInt(
                        pageData.Checksum, false));
