...
namespace TagLib.Ogg
{
    ...
    public class XiphComment : Tag, IEnumerable<string>
    {
        ...
        /// </summary>
        private readonly Dictionary<string, string[]> _fieldList =
        ...
        ///    cref="Comment" />.
        /// </summary>
        private string _commentField = "DESCRIPTION";
        ...
        public string[] GetField(string key)
        {
            ...
            if (!_fieldList.ContainsKey(key))
                return new string[0];

            return (string[])_fieldList[key].Clone();
        }
        ...
        public void SetField(string key, params string[] values)
        {
            ...
            if (result.Count == 0)
                RemoveField(key);
            else if (_fieldList.ContainsKey(key))
                _fieldList[key] = result.ToArray();
            else
                _fieldList.Add(key, result.ToArray());
        }
        ...
        public ByteVector Render(bool addFramingBit)
        {
            ...
            ByteVector vendorData = ByteVector.FromString(
                _vendorId, StringType.Utf8);

            data.Add(ByteVector.FromUInt((uint)vendorData.Count,
                false));
            data.Add(vendorData);
            ...
            foreach (KeyValuePair<string, string[]> entry in _fieldList)
            {
                ...
                foreach (string value in entry.Value)
                {
                    ...
                    fieldData.Add((byte)'=');
                    fieldData.Add(ByteVector.FromString(
                        value, StringType.Utf8));

                    data.Add(ByteVector.FromUInt((uint)
                        fieldData.Count, false));
                    data.Add(fieldData);
                }
            }
        ...
        public uint FieldCount
        {
            ...
            {
                uint count = 0;
                foreach (string[] values in _fieldList.Values)
        ...
        protected void Parse(ByteVector data)
        {
            ...
            int pos = 0;
            int vendorLength = (int)data.Mid(pos, 4)
                .ToUInt(false);
            pos += 4;

            _vendorId = data.ToString(StringType.Utf8, pos,
                vendorLength);
            pos += vendorLength;

            // Next the number of fields in the comment vector.

            int commentFields = (int)data.Mid(pos, 4)
                .ToUInt(false);
            pos += 4;
            ...
            for (int i = 0; i < commentFields; i++)
            {
                ...
                int commentLength = (int)data.Mid(pos, 4)
                    .ToUInt(false);
                pos += 4;
                ...
                int commentSeparatorPosition = comment
                    .IndexOf('=');
                ...
                string value = comment.Substring(
                    commentSeparatorPosition + 1);
                string[] values;

                if (_fieldList.TryGetValue(key, out values))
                {
                    Array.Resize<string>(ref values,
                        values.Length + 1);
                    values[values.Length - 1] = value;
                    _fieldList[key] = values;
                }
                else
                {
                    SetField(key, value);
        ...
        {
            return _fieldList.Keys.GetEnumerator();
        }
        ...
        public override string Comment
        {
            get
            {
                string value = GetFirstField(_commentField);
                if (value != null || _commentField == "COMMENT")
                    return value;

                _commentField = "COMMENT";
                return GetFirstField(_commentField);
            }
            set { SetField(_commentField, value); }
        }
