...
namespace TagLib.Mpeg4
{
    ...
    public class AppleElementaryStreamDescriptor : FullBox
    {
        ...
        /// </summary>
        private readonly ushort _esId;

        /// <summary>
        ///    Contains the stream priority.
        /// </summary>
        private readonly byte _streamPriority;

        /// <summary>
        ///    Contains the object type ID.
        /// </summary>
        private readonly byte _objectTypeId;
        ...
        /// </summary>
        private readonly uint _bufferSizeDb;

        /// <summary>
        ///    Contains the maximum bitrate.
        /// </summary>
        private readonly uint _maxBitrate;
        ...
        /// </summary>
        private readonly ByteVector _decoderConfig;
        ...
        {
            int offset = 0;
            ByteVector boxData = file.ReadBlock(DataSize);
            _decoderConfig = new ByteVector();
            ...
            if (boxData[offset++] == 3)
            {
                ...
                // least 20 long.
                if (ReadLength(boxData, ref offset) < 20)
                    throw new CorruptFileException(
                        "Insufficient data present.");

                _esId = boxData.Mid(offset, 2).ToUShort();
                offset += 2;
                _streamPriority = boxData[offset++];
            }
            else
            {
            ...
            if (ReadLength(boxData, ref offset) < 15)
                throw new CorruptFileException(
                    "Could not read data. Too small.");

            // Read a lot of good info.
            _objectTypeId = boxData[offset++];
            _streamType = boxData[offset++];
            _bufferSizeDb = boxData.Mid(offset, 3).ToUInt();
            offset += 3;
            _maxBitrate = boxData.Mid(offset, 4).ToUInt();
            offset += 4;
            _averageBitrate = boxData.Mid(offset, 4).ToUInt();
            offset += 4;
            ...
            uint length = ReadLength(boxData, ref offset);
            _decoderConfig = boxData.Mid(offset, (int)length);
        }
        ...
        /// </value>
        public byte StreamPriority => _streamPriority;
        ...
        /// </value>
        public byte ObjectTypeId => _objectTypeId;
        ...
        ///    stream described by the current instance.
        /// </value>
        public byte StreamType => _streamType;
        ...
        /// </value>
        public uint BufferSizeDb => _bufferSizeDb;
        ...
        /// </value>
        public uint MaximumBitrate => _maxBitrate / 1000;
        ...
        ///    bitrate the stream described by the current instance.
        /// </value>
        public uint AverageBitrate => _averageBitrate / 1000;
        ...
        ///    instance.
        /// </value>
        public ByteVector DecoderConfig => _decoderConfig;
