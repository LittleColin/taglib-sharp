...
namespace TagLib.Mpeg4
{
    ...
    public class Box
    {
        ...
        /// </summary>
        private readonly IsoHandlerBox _handler;
        ...
        ///    langword="null" /> if no handler applies.
        /// </value>
        public IsoHandlerBox Handler => _handler;
		...
		*/
        ...
        public Box GetChildRecursively(ByteVector type)
        {
            ...
            foreach (Box box in Children)
            {
                Box childBox = box.GetChildRecursively(type);
                if (childBox != null)
                    return childBox;
            }
		...
		*/
        ...
        /// </value>
        protected int DataSize => (int)(_header.DataSize + _dataPosition -
                                        DataPosition);
        ...
        protected virtual ByteVector Render(ByteVector topData)
        {
            ...
            // be a special case.
            if (freeFound || BoxType == Mpeg4.BoxType.Meta)
            {
                long sizeDifference = DataSize - output.Count;

                // If we have room for free space, add it so we
                // don't have to resize the file.
                if (_header.DataSize != 0 && sizeDifference >= 8)
                    output.Add((new IsoFreeSpaceBox(
                        sizeDifference)).Render());
