...
namespace TagLib
{
    ...
    public class StringCollection : ListBase<string>
    {
        ...
        public static StringCollection Split(string value, string pattern)
        {
            ...
            int position = value.IndexOf(pattern, 0, StringComparison.Ordinal);
            int patternLength = pattern.Length;

            while (position != -1)
            {
                list.Add(value.Substring(previousPosition, position - previousPosition));
                previousPosition = position + patternLength;
                position = value.IndexOf(pattern, previousPosition, StringComparison.Ordinal);
            }

            list.Add(value.Substring(previousPosition));
