...
namespace TagLib.Audible
{
	...
	public class Tag : TagLib.Tag
	{
		...
		/// </value>
		public override string Album => GetTag("provider");
		...
		public string Description => GetTag("description");
		...
		private void Parse(ByteVector data)
		{
			...
			try
			{
				do
				{
					...
					_tags.Add(new KeyValuePair<string, string>(currentKey, currentValue));
		...
		private void SetTag(string tagName, string value)
		{
			...
			{
				if (_tags[i].Key == tagName)
					_tags[i] = new KeyValuePair<string, string>(_tags[i].Key, value);
			}
		}
