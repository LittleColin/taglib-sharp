...
namespace TagLib.Matroska
{
    ...
    public class File : TagLib.File
    {
        ...
        private ulong _durationUnscaled;
...
#pragma warning restore 414
            ...
            : base(abstraction)
        {
            ...
            _properties = new Properties(_duration, codecs);
        }
        ...
        /// </returns>
        public override TagLib.Tag GetTag(TagTypes type,
                                           bool create)
        {
            return null;
        }
        ...
        private void Read(ReadStyle propertiesStyle)
        {
            ...
            while (offset < (ulong)Length)
            {
                ...
                Ebmlid ebmlId = (Ebmlid)element.Id;
                MatroskaId matroskaId = (MatroskaId)element.Id;
...
#pragma warning disable 219 // Assigned, never read
            string tagName = null, tagLanguage = null, tagString = null;
#pragma warning restore 219
            ...
            if (tagName == "AUTHOR")
            {
                _tag.Performers = new string[] { tagString };
            }
            else if (tagName == "TITLE")
            {
                _tag.Title = tagString;
            }
            else if (tagName == "ALBUM")
            {
                _tag.Album = tagString;
            }
            else if (tagName == "COMMENTS")
            {
                _tag.Comment = tagString;
            }
        }
            ...
            while (i < element.DataSize)
            {
                ...
                switch (matroskaId)
                {
                    case MatroskaId.MatroskaDuration:
                        _durationUnscaled = (ulong)child.ReadDouble();
                        if (_timeScale > 0)
                        {
                            _duration = TimeSpan.FromSeconds(_durationUnscaled * _timeScale / 1000000000);
                        }
                        break;

                    case MatroskaId.MatroskaTimeCodeScale:
                        _timeScale = child.ReadUInt();
                        if (_durationUnscaled > 0)
                        {
                            _duration = TimeSpan.FromSeconds(_durationUnscaled * _timeScale / 1000000000);
                        }
                        break;
            ...
            while (i < element.DataSize)
            {
                ...
                switch (matroskaId)
                {
                    case MatroskaId.MatroskaTrackType:
                        {
                            TrackType trackType = (TrackType)child.ReadUInt();
