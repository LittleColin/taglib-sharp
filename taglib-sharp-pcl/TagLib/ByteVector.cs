...
namespace TagLib
{
    ...
    public class ByteVector : IList<byte>, IComparable<ByteVector>
    {
        ...
        {
            get { return _useBrokenLatin1; }
            set { _useBrokenLatin1 = value; }
        }
        ...
        {
            byte[] tmpOut;
            return FromFile(abstraction, out tmpOut, false);
        }
        ...
        {
            byte[] tmpOut;
            return FromPath(path, out tmpOut, false);
        }
        ...
        {
            byte[] tmpOut;
            return FromStream(stream, out tmpOut, false);
        }
        ...
        public int EndsWithPartialMatch(ByteVector pattern)
        {
            ...
            int startIndex = _data.Count - pattern.Count;
            ...
            {
                if (ContainsAt(pattern, startIndex + i, 0,
                    pattern.Count - i))
                {
                    return startIndex + i;
                }
        ...
        public int Find(ByteVector pattern, int offset, int byteAlign)
        {
            ...
            if (offset < 0)
                throw new ArgumentOutOfRangeException(
                    nameof(offset), "offset must be at least 0.");
            ...
            for (int i = 0; i < 256; ++i)
                lastOccurrence[i] = pattern.Count;

            for (int i = 0; i < pattern.Count - 1; ++i)
                lastOccurrence[pattern[i]] = pattern.Count - i - 1;

            for (int i = pattern.Count - 1 + offset; i < _data.Count; i += lastOccurrence[_data[i]])
            {
                int iBuffer = i;
        ...
        public string ToString(StringType type, int offset, int count)
        {
            ...
            ByteVector bom = type == StringType.Utf16 &&
                _data.Count - offset > 1 ? Mid(offset, 2) : null;
        ...
        internal static ByteVector FromFile(File.IFileAbstraction abstraction, out byte[] firstChunk, bool copyFirstChunk)
        {
            ...
            Stream stream = abstraction.ReadStream;
        ...
        internal static ByteVector FromStream(Stream stream, out byte[] firstChunk, bool copyFirstChunk)
        {
            ...
            while (true)
            {
                ...
                int n = stream.Read(bytes, 0, readSize);
                vector.Add(bytes);
                bytesRead += n;
                ...
                if ((bytesRead == stream.Length && stream.Length > 0) || (n < readSize && stream.Length <= 0))
                {
                    break;
                }
            }
