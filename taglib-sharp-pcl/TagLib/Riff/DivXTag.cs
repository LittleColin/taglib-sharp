...
namespace TagLib.Riff
{
    ...
    public class DivXTag : Tag
    {
        ...
        public override string[] Genres
        {
            ...
            {
                string genreName =
                    TagLib.Genres.IndexToVideo(_genre);
        ...
        {
            _title = _artist = _genre = _year = _comment = string.Empty;
            _extraData = new ByteVector(6);
        }
        ...
        public ByteVector Render()
        {
            ...
            data.Add(ByteVector.FromString(_title, StringType.Latin1).Resize(32, 0x20));
            data.Add(ByteVector.FromString(_artist, StringType.Latin1).Resize(28, 0x20));
            data.Add(ByteVector.FromString(_year, StringType.Latin1).Resize(4, 0x20));
            data.Add(ByteVector.FromString(_comment, StringType.Latin1).Resize(48, 0x20));
            data.Add(ByteVector.FromString(_genre, StringType.Latin1).Resize(3, 0x20));
            data.Add(_extraData);
        ...
        {
            _title = data.ToString(StringType.Latin1, 0, 32).Trim();
            _artist = data.ToString(StringType.Latin1, 32, 28).Trim();
            _year = data.ToString(StringType.Latin1, 60, 4).Trim();
            _comment = data.ToString(StringType.Latin1, 64, 48).Trim();
            _genre = data.ToString(StringType.Latin1, 112, 3).Trim();
            _extraData = data.Mid(115, 6);
        }
    }
}