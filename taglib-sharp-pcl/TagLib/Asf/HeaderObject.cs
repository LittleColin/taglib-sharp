...
namespace TagLib.Asf
{
    ...
    public class HeaderObject : Object
    {
        ...
        /// </summary>
        private readonly ByteVector _reserved;
            ...
            : base(file, position)
        {
            ...
            _reserved = file.ReadBlock(2);

            _children.AddRange(file.ReadObjects(childCount,
                file.Tell));
        }
        ...
        public override ByteVector Render()
        {
            ...
            long sizeDiff = (long)output.Count + 30 -
                (long)OriginalSize;

            if (sizeDiff != 0)
            {
                PaddingObject obj = new PaddingObject((uint)
                    (sizeDiff > 0 ? 4096 : -sizeDiff));

                output.Add(obj.Render());
                childCount++;
            }

            output.Insert(0, _reserved);
            output.Insert(0, RenderDWord(childCount));
        ...
        public void RemoveContentDescriptors()
        {
            for (int i = _children.Count - 1; i >= 0; i--)
                if (_children[i].Guid == Asf.Guid.AsfContentDescriptionObject ||
                    _children[i].Guid == Asf.Guid.AsfExtendedContentDescriptionObject)
